...
# ── Lazy load heavy libraries (prevents import errors on cloud) ───────────────
@st.cache_resource
def load_libs():
    import cv2
...
st.markdown(
    ...
    'Real-time hand gesture detection · MediaPipe · No cursor control in browser mode'
...
if libs_ok and source == "📷 Webcam (Live)":
    status_placeholder.markdown(
        '<div class="status-box">⚡ WEBCAM LIVE</div>', unsafe_allow_html=True)
    ...
    if img_file:
        ...
        with mp_hands.Hands(
            ...
            processed, gesture = process_frame(cv2, mp_hands, mp_drawing, frame, hands_model)
        ...
        if gesture:
            ...
            detection_out.markdown(
                '<span class="info-pill">🟢 Hand Detected</span>', unsafe_allow_html=True)
        else:
            gesture_out.markdown(
                '<div class="status-box" style="color:#555;">NO HAND</div>',
                unsafe_allow_html=True)
            detection_out.markdown(
                '<span class="info-pill">🔴 No Detection</span>', unsafe_allow_html=True)
...
# ── Video Upload Mode ─────────────────────────────────────────
    ...
    if uploaded_video:
        status_placeholder.markdown(
            '<div class="status-box">🎞️ PROCESSING VIDEO</div>', unsafe_allow_html=True)
        ...
        with mp_hands.Hands(
            ...
            while cap.isOpened() and not stop_btn:
                ...
                if show_fps:
                    cv2.putText(processed, f"FPS: {fps_val:.1f}",
        ...
        if gesture_counts:
            ...
            for g, count in sorted(gesture_counts.items(), key=lambda x: -x[1]):
                st.markdown(
                    f'<div class="gesture-card"><span>{g}</span> — {count} frames</div>',
                    unsafe_allow_html=True)
...
# ── Image Upload Mode ─────────────────────────────────────────
    ...
    if uploaded_img:
        status_placeholder.markdown(
            '<div class="status-box">🖼️ IMAGE ANALYSIS</div>', unsafe_allow_html=True)
